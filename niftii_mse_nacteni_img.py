...
import csv
import time
#Registrace vsech casovych okamziku, ulozeni do .nii.gz jako 4D - bspline - MSE
def registration_full_bspline_mse(read_path, save_path):
    ...
    for i in range(1, len(time_folders)):
        ...
        bspline_reg.SetOptimizerAsLBFGS2(numberOfIterations=100, solutionAccuracy=2.84E-06, deltaConvergenceTolerance=2.03E-06)
    ...
    save_path_niftii = os.path.join(save_path, 'registered_4D_mse_93044.nii.gz') #nazev souboru, potencionalne jako vstupni argument
    sitk.WriteImage(img_all, save_path_niftii)

    #Ulozeni metrik co csv
    filename = os.path.join(save_path, 'vysledky_metrik_mse_93044.csv')
...
if __name__ =='__main__':
    ...
    registration_full_bspline_mse(dcm_folder, save_floder)
    ...
    with open('cas_niftii_mse_93044.txt', 'w', encoding='utf-8') as f:
        f.write(result)
