...
def registration_full_bspline_mi(read_path, save_path):
    ...
    for i in range(1, len(time_folders)):
        ...
        bspline_reg.SetMetricAsJointHistogramMutualInformation(numberOfHistogramBins=90)
        ...
        bspline_reg.SetOptimizerAsLBFGS2(numberOfIterations=120, solutionAccuracy=1.858675127909574e-08, deltaConvergenceTolerance=7.249428505468357e-09)
        bspline_reg.SetMetricSamplingStrategy(bspline_reg.RANDOM) #Náhodně vybere voxely z obrazu, urychlení
    ...
    save_path_niftii = os.path.join(save_path, 'registered_4D_mi_59716.nii.gz') #nazev souboru, potencionalne jako vstupni argument
    sitk.WriteImage(img_all, save_path_niftii)

    #Ulozeni metrik co csv
    filename = os.path.join(save_path, 'vysledky_metrik_mi_59716.csv')
...
if __name__ =='__main__':
    ...
    registration_full_bspline_mi(dcm_folder, save_floder)
    ...
    with open('cas_niftii_mi_93044.txt', 'w', encoding='utf-8') as f:
        f.write(result)